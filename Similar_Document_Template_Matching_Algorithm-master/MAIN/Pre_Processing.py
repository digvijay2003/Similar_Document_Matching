...
def get_filename(image_path):
    """
    Extracts the filename (without extension) from a given image path.
    ...
    # Split the path at the last directory separator and extract the base name (filename with extension)
    base_name = os.path.basename(image_path)
    # Split the base name at the dot (.) to separate the filename and extension
...
def ensure_folder(folder_path):
    """
    Creates a directory (folder) if it doesn't already exist.

    Args:
        folder_path (str): The path to the directory to create.
    """

    # Use `os.makedirs` to create the directory and any necessary parent directories. The `exist_ok=True` argument prevents errors if the folder already exists.
...
def preprocess_image(image_path, apply_binarization=False, blur_threshold=50):
    ...
    corrected_image, skew_angle = correct_skew(
        img
    )  # Replace with your skew correction implementation
